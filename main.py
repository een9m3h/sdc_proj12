...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    #upsample by 4
    output_l4   = tf.layers.conv2d_transpose(conv_1x1, num_classes, 4, 2, padding='same',
                                           kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    
    #1x1 conv to ensure channel size consistency
    l4_1x1 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, padding='same', 
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3)) ##                                      
    
    #combine at l4 for spatial info transfer
    l4_comb = tf.add(output_l4, l4_1x1)
    
    #upsample by 4
    output_l3   = tf.layers.conv2d_transpose(l4_comb, num_classes, 4, 2, padding='same',
                                           kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ...
    #combine at l3 for spatial info transfer
    l3_comb = tf.add(output_l3, l3_1x1)
    
    #last layer
    output   = tf.layers.conv2d_transpose(l3_comb, num_classes, 16, 8, padding='same',
                                           kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
